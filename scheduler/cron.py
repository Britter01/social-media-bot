...
def run_publisher(ignore_platform_pause: bool = False, only_post_id: str | None = None) -> None:
    ...
    *ignore_platform_pause* — for the bulk paths only. Left False so a bulk
    ...
    for post in due:
        ...
        try:
            # force_platform mirrors ignore_platform_pause: only an explicit
            # Publish Now bypasses the platform publishing pause.
            publisher.publish(post, force_platform=ignore_platform_pause)
        except Exception:
